...
def my_pearson(u, v):
    '''
    计算向量u和v（都打过分数的片断）的相似度（加有significance weighting权重的）并返回
    '''
    ...
    similar = (1 - spatial.distance.correlation(u, v)) * sig_weight if np.unique(u).size > 1 and np.unique(
        v).size > 1 else 0.0
    # similar = (1 - spatial.distance.cosine(u, v)) * sig_weight if np.unique(u).size > 1 and np.unique(v).size > 1 else 0.0
    return similar


def uu_similar(ui_mat_df, NUM_NEIGHBORS):
    '''
    计算所有用户间的相似度, 个性化权重
    '''
    ...
    return uu_similar_array


def predict(uu_similar_array, ui_mat_df):
    '''
    normalization评分预测
    '''
    ...
    return ui_predict_array


def recommend(ui_predict_array, ui_mat_df, NUM_RECOMS):
    '''
    推荐NUM_RECOMS个最可能的items给每个user
    '''
...
if __name__ == '__main__':
    ...
    # 计算所有用户间的相似度users-users-mat
    uu_similar_array = uu_similar(ui_mat_df, NUM_NEIGHBORS)

    # 预测所有users对其它所有items的打分
    ui_predict_array = predict(uu_similar_array, ui_mat_df)

    recommend(ui_predict_array, ui_mat_df, NUM_RECOMS)
